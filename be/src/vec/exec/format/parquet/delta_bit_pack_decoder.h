...
public:
    ...
    Status decode_values(MutableColumnPtr& doris_column, DataTypePtr& data_type,
        ...
        return _fix_plain_decoder->decode_values(doris_column, data_type, select_vector);
    }
    ...
    void set_data(Slice* slice) override {
        _bit_reader.reset(new BatchedBitReader((const uint8_t*)slice->data, slice->size));
    ...
    void set_bit_reader(std::shared_ptr<BatchedBitReader> bit_reader) {
...
public:
    ...
    void set_data(Slice* slice) override {
        ...
        _bit_reader = std::make_shared<BatchedBitReader>((const uint8_t*)slice->data, slice->size);
        _data = slice;
        _offset = 0;
        _decode_lengths();
    }
...
private:
    ...
    std::vector<uint8_t> _buffered_length;
    std::vector<uint8_t> _buffered_data;
};
...
public:
    ...
    Status decode_values(MutableColumnPtr& doris_column, DataTypePtr& data_type,
        ...
        _data = _values.data();
        _type_length = _field_schema->parquet_schema.type_length;
        _data->size = _values.size() * _type_length;
        init_values_converter();
    ...
    void set_data(Slice* slice) override {
        ...
        _buffered_prefix_length.resize(num_prefix * sizeof(int32_t));
        int ret;
        _prefix_len_decoder.decode(reinterpret_cast<int32_t*>(_buffered_prefix_length.data()),
                                   num_prefix, &ret);
...
private:
    ...
    std::vector<uint8_t> _buffered_prefix_length;
    std::vector<uint8_t> _buffered_data;
};
} // namespace doris::vectorized
